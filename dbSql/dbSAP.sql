...
FROM
  (SELECT Ref1 = '',
   ...
   UNION SELECT CAST(T0.Ref1 AS VARCHAR) AS Ref1,
                ...
                T0.TransType AS TipoTrans
   FROM [dbo].[JDT1] T0
   LEFT OUTER JOIN [dbo].[OUSR] T1 ON T1.[USERID] = T0.[UserSign]
   INNER JOIN [dbo].[OJDT] T3 ON T3.[TransId] = T0.[TransId]
   LEFT OUTER JOIN [dbo].[OOAT] T4 ON T4.[AbsID] = T3.[AgrNo]
   LEFT JOIN [dbo].[OINV] T5 ON cast(T5.[Docnum] AS nvarchar(10)) = cast(T0.[Ref1] AS nvarchar(10))
   AND T0.TransType = 13
   LEFT JOIN [dbo].[ORIN] T6 ON cast(T6.[Docnum] AS nvarchar(10)) = cast(T0.[Ref1] AS nvarchar(10))
   AND T0.TransType = 14
   WHERE T0.[ShortName] = ('CXXXXXX')
     AND (T0.[BalDueDeb] <> (0)
          OR T0.[BalDueCred] <> 0
